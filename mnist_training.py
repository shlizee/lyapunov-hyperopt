...
#     data = dl.create_dataset(full_con.data)
    train_loader = torch.utils.data.DataLoader(dataset=fcon.data.datasets['train_set'], batch_size=dcon.batch_size, shuffle=True, pin_memory = pinned)
	val_loader = torch.utils.data.DataLoader(dataset=fcon.data.datasets['val_set'], batch_size=dcon.batch_size, shuffle=True, pin_memory = pinned)
    ...
    for epoch in range(start_epoch+1, full_con.train.max_epoch+1):
        ...
        for train_data, train_target in train_loader:
			batch_in = train_data.flatten(start_dim = 1).squeeze().to(device)
			batch_target = nn.functional.one_hot(train_target, fcon.model.output_size).to(device)
            optimizer.zero_grad()
            loss = 0.0
